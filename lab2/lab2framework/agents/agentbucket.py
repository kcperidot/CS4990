...
class BucketHatPlayer(agent.Agent):
    ...
    def get_action(self, nr, hands, knowledge, trash, played, board, valid_actions, hints, hits, cards_left):
        ...
        if hints > 0:
            hints = util.filter_actions(HINT_COLOR, valid_actions) + util.filter_actions(HINT_RANK, valid_actions)
            hintgiven = random.choice(hints) #change to not be random
        ...
        if potential_discards:
            return Action(DISCARD, card_index=random.choice(potential_discards)) #change not to be random
        ...
        while playables and hints > 0:
            ...
            t = None
            if hinttype:
                t = random.choice(hinttype) #change random
        ...
        return random.choice(util.filter_actions(DISCARD, valid_actions)) #remove random
